...
async function loadDictionary() {
  ...
  dictionaryPromise = (async () => {
    try {
      // CDNから辞書ファイルを読み込む
      const affResponse = await fetch('https://cdn.jsdelivr.net/npm/typo-js@1.3.1/dictionaries/en_US/en_US.aff')
      const dicResponse = await fetch('https://cdn.jsdelivr.net/npm/typo-js@1.3.1/dictionaries/en_US/en_US.dic')
...
function checkSpelling(doc: any): DecorationSet {
  ...
  while ((match = wordRegex.exec(text)) !== null) {
    ...
    if (!dictionary.check(word)) {
      // 一般的な略語や固有名詞は除外
      const commonWords = ['url', 'api', 'id', 'ui', 'ux', 'app', 'ok', 'etc', 'vs', 'http', 'https']
